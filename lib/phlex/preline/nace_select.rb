...
module Phlex
  module Preline
    # NaceSelect - NACE Industry Selection Component
    #
    # A flexible component for selecting NACE (European) industries with support for both
    # single and multiple selection modes and multi-language support.
    #
    # @example Multiple selection (default)
    #   NaceSelect.new(
    #     field_name: "target_criteria[nace_codes]",
    #     label: "Industries",
    #     selected: positioning.nace_codes,
    #     multiple: true,
    #     language: 'en'
    #   )
    #
    # @example Single selection with French language
    #   NaceSelect.new(
    #     field_name: "primary_nace_code",
    #     label: "Primary Industry",
    #     selected: company.primary_nace_code,
    #     multiple: false,
    #     language: 'fr'
    #   )
    #
    class NaceSelect < Phlex::HTML
      def initialize(
        field_name:,
        label: "Industries",
        selected: [],
      ...
      def view_template
        div(
          class: "nace-select-wrapper space-y-2",
          data: {
            controller: "nace-select",
            nace_select_multiple_value: @multiple,
            nace_select_hierarchical_value: @hierarchical,
            nace_select_max_selections_value: @max_selections,
            nace_select_field_name_value: @field_name,
            nace_select_language_value: @language
          }
        ) do
          render_label if @label

          # Selected industries container
          div(data: { "nace-select-target" => "selectedLabelsContainer" }) do
            if @selected.any?
      ...
      def render_search_input
        div(class: "relative") do
          input(
            ...
            data: {
              nace_select_target: "searchInput",
              action: "input->nace-select#search"
            }
          )
          div(
      ...
      def render_selection_display
        div(class: "mb-4") do
          div(class: "flex flex-wrap gap-2 mb-3") do
            selected_codes = @data_provider ? @data_provider.find_nace_by_codes(@selected) : find_nace_codes_fallback
            selected_codes.each do |nace_code|
              render_selected_item(nace_code)
            end
            ...
            if @multiple
              button(
                ...
                data: {
                  action: "click->nace-select#toggleSelector",
                  "nace-select-target" => "toggleButton"
                }
              ) do
                ...
                span(data: { "nace-select-target" => "toggleText" }) { "Add more industries" }
              end
            end
          end
      ...
      def render_selected_item(nace_code)
        render Components::Preline::Badge.new(
          text: nace_code.display_name(@language),
          variant: :primary,
          size: :sm,
          pill: true,
          removable: true,
          data: {
            code: nace_code.code,
            nace_name: nace_code.display_name(@language),
            action: "click->nace-select#removeSelection"
          }
        )
      end
      ...
      def render_select_button
        button(
          ...
          span(data: { "nace-select-target" => "toggleText" }) { "Select industries" }
        end
      end

      def render_nace_tree
        div(
          class: "hidden nace-tree border rounded-lg mt-4 p-4 bg-gray-50",
          data: { nace_select_target: "naceTree" }
        ) do
      ...
      def render_hierarchical_tree
        div(class: "p-2") do
          if @data_provider && @data_provider.respond_to?(:get_hierarchical_data)
            hierarchy = @data_provider.get_hierarchical_data(system: :nace, language: @language)
            hierarchy.each do |section_data|
              render_tree_node_from_data(section_data, level: 0)
            end
          elsif defined?(NaceCode)
            NaceCode.sections.includes(:children).order(:code).each do |section|
              render_tree_node(section, level: 0)
            end
          else
            div(class: "p-4 text-center text-gray-500") do
              plain "NACE data not available"
            end
          end
        end
      end
      ...
      def render_tree_node_from_data(node_data, level: 0)
        ...
        div(class: "select-none") do
          div(
            ...
            data: {
              action: "click->nace-select#selectItem",
      ...
      def render_tree_node(nace_code, level: 0)
        return if @selected.include?(nace_code.code)

        indent_class = "ml-#{level * 4}"
        has_children = nace_code.children.any?
        has_available_children = has_children && nace_code.children.any? { |child| !@selected.include?(child.code) }

        div(class: "nace-node #{indent_class}", data: { code: nace_code.code, level: level }) do
          div(
            class: "flex items-center p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded cursor-pointer",
            data: {
              action: "click->nace-select#toggleSelection",
              code: nace_code.code
            }
          ) do
            # Expansion indicator
            div(class: "w-6 flex justify-center items-center") do
              if has_available_children
                button(
                  ...
                  data: {
                    action: "click->nace-select#toggleExpansion",
                    code: nace_code.code
                  }
                ) do
                  svg(
                    ...
                    fill: "currentColor",
                    viewBox: "0 0 20 20",
                    data: { nace_select_target: "expandIcon" }
                  ) do |s|
                    s.path(
                      fill_rule: "evenodd",
            ...
            if @show_selection_indicators
              div(
                ...
                data: { nace_select_target: "selectionIndicator" }
              ) do
                if @multiple
                  input(
                    type: "checkbox",
            ...
            # NACE title and description
            div(class: "flex-1") do
              div(class: "text-sm font-medium text-gray-900 dark:text-gray-100") do
                plain nace_code.title(@language)
              end
              if nace_code.description(@language).present?
                div(class: "text-xs text-gray-500 dark:text-gray-400 mt-1") do
                  plain truncate(nace_code.description(@language), length: 100)
                end
              end
            end
          end
          ...
          if has_children
            div(
              ...
              style: "display: none;",
              data: {
                nace_select_target: "childrenContainer",
                parent_code: nace_code.code
              }
            ) do
              nace_code.children.order(:code).each do |child|
      ...
      def render_flat_list
        div(class: "p-2 space-y-1") do
          if @data_provider && @data_provider.respond_to?(:find_by_codes)
            # Get all NACE codes from service - we'll flatten the hierarchy
            hierarchy = @data_provider.get_hierarchical_data(system: :nace, language: @language)
            ...
            NaceCode.all.order(:level, :code).each do |nace_code|
              render_flat_item(nace_code)
            end
          else
            div(class: "p-4 text-center text-gray-500") do
              plain "NACE data not available"
      ...
      def render_flat_item(nace_code)
        is_selected = @selected.include?(nace_code.code)
          ...
          div(class: "flex-1") do
            div(class: "text-sm font-medium text-gray-900 dark:text-gray-100") do
              span(class: "inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-100 text-gray-800 mr-2") do
                plain nace_code.level.humanize
              end
              plain nace_code.title(@language)
            end
          end
        ...
        if @multiple
          @selected.each do |code|
            input(
              ...
              value: code,
              data: {
                nace_select_target: "hiddenInput",
                code: code
              }
            )
          end
          ...
          input(
            ...
            data: { nace_select_target: "hiddenInput" }
          )
        end
      end
      ...
      def find_nace_codes_fallback
        if @data_provider && @data_provider.respond_to?(:find_by_codes)
          @data_provider.find_by_codes(@selected, system: :nace)
        elsif defined?(NaceCode)
          NaceCode.where(code: @selected)
        else
          []
        end
      end
